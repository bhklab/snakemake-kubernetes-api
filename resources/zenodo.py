...
def upload(object, source_dir):
    ...
    try:
        ...
        if data['create_repo']:
            ...
            metadata = {
                'metadata': {
                    'title': object.pipeline_name,
                    'upload_type': 'dataset',
                    'description': object.pipeline_name + ' data object generated by ORCESTRA.',
        ...
        if data['add_metadata']:
            # Upload
            res = None
            with open('{0}/{1}'.format(source_dir, object.filename), 'rb') as fp:
                res = requests.put(
                    bucket_url + '/' + object.filename,
                    data=fp,
                    params={'access_token': ACCESS_TOKEN}
                )
            print('upload data: %s' % res.status_code)
            if res.status_code == 200:
                data['download_link'] = BASE_URL + "/record/" + str(deposition_id) + "/files/" + object.filename + "?download=1"
                data['upload'] = True
            else:
                data['error'] = res.json()
